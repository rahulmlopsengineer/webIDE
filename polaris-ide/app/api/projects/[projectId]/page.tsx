...
type RightPanel = 'chat' | 'agent' | 'preview' | 'db';
...
export default function ProjectEditorPage({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const files: ApiFile[] = (data.files ?? []).map((f: ApiFile) => ({
  ...
  const handleAgentFileChange = useCallback((filePath: string, fileName: string, content: string) => {
    ...
    setProject((prev) => {
      ...
      if (fileExists) {
        return { ...prev, files: prev.files.map((f) =>
  ...
  }, [openTabs, projectId, saveToDb, createDbFile]);

  // ── Boot WebContainer ─────────────────────────────────
   const handleBoot = useCallback(() => {
    if (!project) return;

    // Merge saved files + any unsaved tab edits
    const files: Record<string, string> = {};
    project.files.forEach((f) => { files[f.filePath] = f.content; });
    openTabs.forEach((t) => { files[t.path] = t.content; });

    // Static HTML/CSS/JS and vanilla-js projects are served by a tiny
    // inline Node.js server written directly into the container by
    // use-webcontainer.ts — no package.json injection needed.
    // Only inject package.json for Next.js and React (they need npm install).
    const fw = project.framework;
    if (fw === 'nextjs' || fw === 'react') {
      const devCmd = fw === 'nextjs'
        ? 'next dev --port 3000'
        : 'vite --port 3000 --host';
      try {
        const existing = files['package.json'] ? JSON.parse(files['package.json']) : {};
        files['package.json'] = JSON.stringify({
          ...existing,
          name:    ((existing.name as string) || project.name).toLowerCase().replace(/[^a-z0-9-]/g, '-').slice(0, 40) || 'app',
          version: (existing.version as string) || '0.1.0',
          private: true,
          scripts: { ...((existing.scripts as Record<string,string>) ?? {}), dev: devCmd },
        }, null, 2);
      } catch {
        files['package.json'] = JSON.stringify({
    ...
    wc.boot(files, project.framework, projectId);
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-bg-0 text-text font-sans">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Right panel */}
        <div className="flex flex-col w-[300px] flex-shrink-0 border-l border-border overflow-hidden">
          {rightPanel === 'chat' && (
